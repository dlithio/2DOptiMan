function myreturn = writeVTK(pointsUsed2,index2,data)
...
for timeStep = 1:(length(pointsUsed2)-1)
    for local = 1:pointsUsed2(timeStep)
        ...
        localNextRing = getLocalNextRing(local,timeStep,index2);
        nextLocalNextRing = getNextLocalNextRing(local,timeStep,pointsUsed2,index2);
        beforeNextLocalNextRing = getBeforeNextLocalNextRing(local,timeStep,pointsUsed2,index2);
        %Global point refs
